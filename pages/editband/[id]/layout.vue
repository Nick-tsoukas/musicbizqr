...
async function saveLayout() {
  ...
  try {
    const layoutConfig = createSaveableConfig({
      version: 1,
    ...
    await $fetch(`${config.public.strapiUrl}/api/bands/${bandId.value}`, {
      method: 'PUT',
      headers: {
    ...
    saveError.value = 'Failed to save layout. Please try again.';
  } finally {
    saving.value = false;
  }
}
...
async function resetToDefault() {
  ...
  try {
    ...
    saveError.value = 'Failed to reset layout. Please try again.';
  } finally {
    saving.value = false;
  }
}
